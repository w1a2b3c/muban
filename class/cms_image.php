...
final class cms_image
{
    ...
    function get_postion($a,$source,$water)
    {
    ...
    #生成缩略图
    function thumb($old,$width=200,$height=200,$new='')
    {
        if(!self::check())
        {
            return WEB_ROOT.$old;
    ...
    #原图等比压缩
    function create_thumb($old,$width=600)
    {
        if(!self::check())
        {
            return;
    ...
    #添加水印
    function watermark($old)
    {
		$water_logo=config('water_logo');
        if(empty($water_logo)){return;}
		
        $watermark=SYS_PATH.$water_logo;
        ...
        if(is_array($source) && is_array($water))
        {
            $createtype=$source[2];
            $savetype=$source[3];
            if($source[0]<config('water_width')||$source[1]<config('water_height')){return;}
            #原图小于水印图片则不加
            if($source[0]<$water[0] || $source[1]<$water[1]){return;}
            $water_img=$water[2]($watermark);
            $source_img=$source[2]($old);
            $postion=$this->get_postion(config('water_position'),$source,$water);
            $x=$postion[0];
            $y=$postion[1];
